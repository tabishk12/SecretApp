...
app.get("/", async (req, res) => {
    try {
        ...
        res.render("home", { user:loggedInUser ,loggedIn: req.session.loggedIn, users: users, secrets: secrets });
    } catch (error) {
        console.error(error);
        res.render("error");
    }
...
app.post('/register', async (req, res) => {
    ...
    try {
        const newUser = new User({fullName:fullName, dob:dob,name: username, password, secrets: [] });
        await newUser.save();
...
app.get("/contact", async (req, res) => {
    ...
    try {
        ...
        res.render("contact", { reviews: reviews,user:loggedInUser });
    } catch (error) {
        console.error(error);
        res.render("error");
    }
...
app.get("/compose", (req, res) => {
    const loggedInUser = req.session.userId ? User.findById(req.session.userId) : null;
    res.render("compose",{user: loggedInUser});
});
...
app.get("/about", async (req, res) => {
    const loggedInUser = req.session.userId ? await User.findById(req.session.userId) : null;
    res.render("about", { user: loggedInUser });
});
app.get("/secret", async (req, res) => {
    ...
    try {
        ...
        res.render("posts", {user: loggedInUser, posts: secrets });
    } catch (error) {
...
app.post("/delete/user/:userId", async (req, res) => {
    ...
    try {
        // Implement logic to delete the user from the database based on the userId
        await User.findByIdAndDelete(userId);
        
        // Redirect the user to a relevant page after successful deletion
        res.redirect('/logout'); // For example, redirecting to the logout page
    } catch (error) {
